...
#define TwoSTEPs256(j) \
	...
	TT[1] = _mm_add_epi32( TT[1], TT[3]);\
	\
	TT[2] = _mm_shufflehi_epi16(TT[2],177);\
	Temp = _mm_and_si128(TT[2], M2);\
	Temp = _mm_xor_si128(_mm_slli_epi32(Temp, 8), _mm_srli_epi32(Temp, 24));\
	TT[2] = _mm_xor_si128(_mm_andnot_si128(M2,TT[2]),Temp);\
	TT[2] = _mm_shuffle_epi32( TT[2], 114);\
	\
	TT[3] = _mm_shufflehi_epi16(TT[3],30);\
	TT[3] = _mm_shufflelo_epi16(TT[3],180);\
	Temp = _mm_and_si128(TT[3], M1);\
	Temp = _mm_xor_si128(_mm_slli_epi32(Temp, 8), _mm_srli_epi32(Temp, 24));\
	TT[3] = _mm_xor_si128(_mm_andnot_si128(M1,TT[3]),Temp);\
	\
	\
	...
	TT[1] = _mm_add_epi32( TT[1], TT[0]);\
	\
	TT[0] = _mm_shufflehi_epi16(TT[0],177);\
	Temp = _mm_and_si128(TT[0], M2);\
	Temp = _mm_xor_si128(_mm_slli_epi32(Temp, 8), _mm_srli_epi32(Temp, 24));\
	TT[0] = _mm_xor_si128(_mm_andnot_si128(M2,TT[0]),Temp);\
	Temp = _mm_shuffle_epi32( TT[0],114);\
	TT[0] = _mm_add_epi32( TT[3], TT[2]);\
	TT[3] = Temp;\
	\
	TT[2] = _mm_shufflehi_epi16(TT[2],30);\
	TT[2] = _mm_shufflelo_epi16(TT[2],180);\
	Temp = _mm_and_si128(TT[2], M1);\
	Temp = _mm_xor_si128(_mm_slli_epi32(Temp, 8), _mm_srli_epi32(Temp, 24));\
	Temp = _mm_xor_si128(_mm_andnot_si128(M1,TT[2]),Temp);\
	TT[2] = _mm_shuffle_epi32( TT[1], 210);\
	TT[1] = Temp;\
	\
	\
...
void compress256(hashState256 * state, const Byte * datablock) {
	ALIGN(16) u32 m[32];
	ALIGN(16) const u32 Mask1[4] = {0xFFFFFFFF,0xFFFFFFFF,0,0};
	ALIGN(16) const u32 Mask2[4] = {0,0xFFFFFFFF,0,0xFFFFFFFF};
	...
	Temp = _mm_xor_si128(_mm_slli_epi32(Temp, 8), _mm_srli_epi32(Temp, 24));
	TT[0] = _mm_xor_si128(_mm_andnot_si128(M2,TT[0]),Temp);
	TT[0] = _mm_shuffle_epi32( TT[0],114);
	Temp = _mm_xor_si128( TT[0], M[3]);
	TT[0] = TT[3];
	TT[3] = Temp;
	...
	Temp = _mm_and_si128(TT[2], M1);
	Temp = _mm_xor_si128(_mm_slli_epi32(Temp, 8), _mm_srli_epi32(Temp, 24));
	TT[2] = _mm_xor_si128(_mm_andnot_si128(M1,TT[2]),Temp);
	Temp = _mm_xor_si128( TT[2], M[1]);
	TT[2] = TT[1];
	TT[1] = Temp;
	
	//store
	_mm_store_si128((__m128i*)(&state->cv256[0]),TT[0]);
	_mm_store_si128((__m128i*)(&state->cv256[4]),TT[1]);
	_mm_store_si128((__m128i*)(&state->cv256[8]),TT[2]);
	_mm_store_si128((__m128i*)(&state->cv256[12]),TT[3]);
	return;
}
