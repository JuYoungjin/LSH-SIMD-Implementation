...
#define TwoSTEPs256(j); \
	...
	TT[0] = _mm_roti_epi32( TT[0], 29);\
	TT[0] = _mm_xor_si128( TT[0], _mm_load_si128((__m128i*)(&SC256[j][0])));\
	\
	TT[1] = _mm_add_epi32( TT[1], TT[3]);\
	TT[1] = _mm_roti_epi32( TT[1], 29);\
	TT[1] = _mm_xor_si128( TT[1], _mm_load_si128((__m128i*)(&SC256[j][4])));\
	\
	TT[2] = _mm_add_epi32( TT[2], TT[0]);\
	TT[2] = _mm_roti_epi32( TT[2], 1);\
	\
	TT[3] = _mm_add_epi32( TT[3], TT[1]);\
	TT[3] = _mm_roti_epi32( TT[3], 1);\
	\
	TT[0] = _mm_add_epi32( TT[0], TT[2]);\
	TT[1] = _mm_add_epi32( TT[1], TT[3]);\
	\
	TT[0] = _mm_perm_epi8(TT[0],TT[0], selector[0]);\
	TT[2] = _mm_perm_epi8(TT[2],TT[2], selector[1]);\
	TT[3] = _mm_perm_epi8(TT[3],TT[3], selector[2]);\
	\
	...
	TT[1] = _mm_roti_epi32( TT[1], 5);\
	TT[1] = _mm_xor_si128( TT[1], _mm_load_si128((__m128i*)(&SC256[j+1][0])));\
	\
	TT[3] = _mm_add_epi32( TT[3], TT[2]);\
	TT[3] = _mm_roti_epi32( TT[3], 5);\
	TT[3] = _mm_xor_si128( TT[3], _mm_load_si128((__m128i*)(&SC256[j+1][4])));\
	\
	TT[0] = _mm_add_epi32( TT[0], TT[1]);\
	TT[0] = _mm_roti_epi32( TT[0], 17);\
	\
	TT[2] = _mm_add_epi32( TT[2], TT[3]);\
	TT[2] = _mm_roti_epi32( TT[2], 17);\
	\
	TT[1] = _mm_add_epi32( TT[1], TT[0]);\
	TT[3] = _mm_add_epi32( TT[3], TT[2]);\
	\
	Temp = TT[2];\
	TT[2] = _mm_perm_epi8(TT[1], TT[1], selector[0]);\
	TT[1] = _mm_perm_epi8(Temp, Temp, selector[2]);\
	Temp = TT[3];\
	TT[3] = _mm_perm_epi8(TT[0], TT[0], selector[1]);\
	TT[0] = Temp;\
	\
...
void compress256(hashState256 * state, const Byte * datablock) {
	ALIGN(16) u32 m[32];
	ALIGN(16) const u64 s1[2] = {0x030201000b0a0908ULL, 0x0f0e0d0c07060504ULL};
	ALIGN(16) const u64 s2[2] = {0x0302010009080b0aULL, 0x060504070c0f0e0dULL};
	ALIGN(16) const u64 s3[2] = {0x0407060502010003ULL, 0x09080b0a0f0e0d0cULL};
	...
	M[0] = _mm_add_epi32(_mm_shuffle_epi32( M[0], 75), M[4]);
	M[1] = _mm_add_epi32(_mm_shuffle_epi32( M[1], 30), M[5]);
	M[2] = _mm_add_epi32(_mm_shuffle_epi32( M[2], 75), M[6]);
	M[3] = _mm_add_epi32(_mm_shuffle_epi32( M[3], 30), M[7]);
	...
	TT[1] = _mm_perm_epi8(TT[1], TT[1], selector[0]);
	TT[2] = _mm_perm_epi8(TT[2], TT[2], selector[2]);
			
	TT[0] = _mm_perm_epi8(TT[0], TT[0], selector[1]);
...
void Update256(hashState256 * state, const Byte * data, DataLength databitlen) {
	...
	*/
	if ((u32)(databitlen & 0x3ff)){
		temp = (numBlocks) << 7; //temp = 128*(numBlocks-1)
	...
	// end: computation of the Last256 block
...
void Final256(hashState256 * state, Byte * hashval) {
	...
	for (j = 0; j < (state->hashbitlen) >> 3; j++){
//		
		hashval[j] = (u8)(H[j >> 2] >> ((j << 3) & 0x1f)); //0,8,16,24,0,,.. = 8*l (mod 32) = (l<<3)&0x1f
	}
