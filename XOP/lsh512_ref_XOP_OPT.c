...
#define TwoSTEPs512(j) \
	...
	TT[3] = _mm_add_epi64( TT[3], TT[7]);\
	\
	Temp[0] = _mm_unpacklo_epi64(TT[1],TT[0]);\
	TT[1] = _mm_unpackhi_epi64(TT[0],TT[1]);\
	TT[0] = Temp[0];\
	Temp[0] = _mm_perm_epi8(TT[4],TT[5], selector[0]);\
	TT[5] = _mm_perm_epi8(TT[4],TT[5], selector[1]);\
	TT[4] = Temp[0];\
	TT[6] = _mm_rot_epi64(TT[6], selector[2]);\
	TT[7] = _mm_perm_epi8(TT[7],TT[7], selector[3]);\
	\
	Temp[0] = _mm_shuffle_epi32( M[1], 78);\
	M[1] = _mm_add_epi64(M[9], M[0]);\
	M[0] = _mm_add_epi64(M[8], Temp[0]);\
	\
	Temp[0] = _mm_shuffle_epi32( M[2], 78);\
	M[2] = _mm_add_epi64(M[3], M[10]);\
	M[3] = _mm_add_epi64(M[11], Temp[0]);\
	\
	Temp[0] = _mm_shuffle_epi32( M[5], 78);\
	M[5] = _mm_add_epi64(M[13], M[4]);\
	M[4] = _mm_add_epi64(M[12],Temp[0]);\
	\
	Temp[0] = _mm_shuffle_epi32( M[6], 78);\
	M[6] = _mm_add_epi64(M[14], M[7]);\
	M[7] = _mm_add_epi64(M[15], Temp[0]);\
	\
	\
	TT[2] = _mm_xor_si128( TT[2], M[8]);\
	TT[3] = _mm_xor_si128( TT[3], M[9]);\
	TT[6] = _mm_xor_si128( TT[6], M[10]);\
	TT[7] = _mm_xor_si128( TT[7], M[11]);\
	TT[0] = _mm_xor_si128( TT[0], M[12]);\
	TT[1] = _mm_xor_si128( TT[1], M[13]);\
	TT[4] = _mm_xor_si128( TT[4], M[14]);\
	TT[5] = _mm_xor_si128( TT[5], M[15]);\
	\
	TT[2] = _mm_add_epi64( TT[2], TT[0]);\
	TT[2] = _mm_roti_epi64( TT[2], 7);\
	TT[2] = _mm_xor_si128( TT[2], _mm_load_si128((__m128i*)(&SC512[j+1][0])));\
	\
	TT[3] = _mm_add_epi64( TT[3], TT[1]);\
	TT[3] = _mm_roti_epi64( TT[3], 7);\
	TT[3] = _mm_xor_si128( TT[3], _mm_load_si128((__m128i*)(&SC512[j+1][2])));\
	\
	TT[6] = _mm_add_epi64( TT[6], TT[4]);\
	TT[6] = _mm_roti_epi64( TT[6], 7);\
	TT[6] = _mm_xor_si128( TT[6], _mm_load_si128((__m128i*)(&SC512[j+1][4])));\
	\
	TT[7] = _mm_add_epi64( TT[7], TT[5]);\
	TT[7] = _mm_roti_epi64( TT[7], 7);\
	TT[7] = _mm_xor_si128( TT[7], _mm_load_si128((__m128i*)(&SC512[j+1][6])));\
	\
	TT[0] = _mm_add_epi64( TT[0], TT[2]);\
	TT[0] = _mm_roti_epi64( TT[0], 3);\
	\
	TT[1] = _mm_add_epi64( TT[1], TT[3]);\
	TT[1] = _mm_roti_epi64( TT[1], 3);\
	\
	TT[4] = _mm_add_epi64( TT[4], TT[6]);\
	TT[4] = _mm_roti_epi64( TT[4], 3);\
	\
	TT[5] = _mm_add_epi64( TT[5], TT[7]);\
	TT[5] = _mm_roti_epi64( TT[5], 3);\
	\
	TT[2] = _mm_add_epi64( TT[2], TT[0]);\
	TT[3] = _mm_add_epi64( TT[3], TT[1]);\
	TT[6] = _mm_add_epi64( TT[6], TT[4]);\
	TT[7] = _mm_add_epi64( TT[7], TT[5]);\
	\
	Temp[0] =_mm_unpacklo_epi64(TT[3],TT[2]);\
	Temp[1] = _mm_unpackhi_epi64(TT[2],TT[3]);\
	\
	TT[2] = _mm_rot_epi64(TT[4], selector[2]);\
	TT[3] = _mm_perm_epi8(TT[5],TT[5], selector[3]);\
	\
	TT[4] = Temp[0];\
	TT[5] = Temp[1];\
	\
	Temp[0] = _mm_perm_epi8(TT[0], TT[1], selector[0]);\
	Temp[1] = _mm_perm_epi8(TT[0], TT[1], selector[1]);\
	\
	TT[0] = TT[6];\
	TT[1] = TT[7];\
	\
	...
	TT[7] = Temp[1];\
	\
	Temp[0] = _mm_shuffle_epi32( M[9], 78);\
	M[9] = _mm_add_epi64(M[1], M[8]);\
	M[8] = _mm_add_epi64(M[0], Temp[0]);\
	\
	Temp[0] = _mm_shuffle_epi32( M[10], 78);\
	M[10] = _mm_add_epi64(M[11], M[2]);\
	M[11] = _mm_add_epi64(M[3], Temp[0]);\
	\
	Temp[0] = _mm_shuffle_epi32( M[13], 78);\
	M[13] = _mm_add_epi64(M[5], M[12]);\
	M[12] = _mm_add_epi64(M[4],Temp[0]);\
	\
	Temp[0] = _mm_shuffle_epi32( M[14], 78);\
	M[14] = _mm_add_epi64(M[6], M[15]);\
	M[15] = _mm_add_epi64(M[7], Temp[0]);\
...
void compress512(hashState512 * state, const Byte * datablock) {
	int j;
	ALIGN(16) u64 m[32];
	ALIGN(16) const u64 s1[2] = {0x1312111017161514ULL, 0x0706050403020100ULL};
	ALIGN(16) const u64 s2[2] = {0x19181f1e1d1c1b1aULL, 0x0d0c0b0a09080f0eULL};
	ALIGN(16) const u64 s3[2] = {0x0000000000000028ULL, 0x0000000000000008ULL};
	ALIGN(16) const u64 s4[2] = {0x080f0e0d0c0b0a09ULL, 0x0403020100070605ULL};
	__m128i M[16];
	__m128i TT[8];
	__m128i Temp[2];
	__m128i selector[4];
	selector[0] = _mm_load_si128((__m128i*)s1);	
	selector[1] = _mm_load_si128((__m128i*)s2);	
	selector[2] = _mm_load_si128((__m128i*)s3);
	selector[3] = _mm_load_si128((__m128i*)s4);

	m[0] = U8TO64_LE(datablock);		m[16] = U8TO64_LE(datablock+128);
	m[1] = U8TO64_LE(datablock+8);		m[17] = U8TO64_LE(datablock+136);
	m[2] = U8TO64_LE(datablock+16);		m[18] = U8TO64_LE(datablock+144);
	m[3] = U8TO64_LE(datablock+24);		m[19] = U8TO64_LE(datablock+152);
	m[4] = U8TO64_LE(datablock+48);		m[20] = U8TO64_LE(datablock+176);
	m[5] = U8TO64_LE(datablock+32);		m[21] = U8TO64_LE(datablock+160);
	m[6] = U8TO64_LE(datablock+40);		m[22] = U8TO64_LE(datablock+168);
	m[7] = U8TO64_LE(datablock+56);		m[23] = U8TO64_LE(datablock+184);
	m[8] = U8TO64_LE(datablock+64);		m[24] = U8TO64_LE(datablock+192);
	m[9] = U8TO64_LE(datablock+72);		m[25] = U8TO64_LE(datablock+200);
	m[10] = U8TO64_LE(datablock+80);	m[26] = U8TO64_LE(datablock+208);
	m[11] = U8TO64_LE(datablock+88);	m[27] = U8TO64_LE(datablock+216);
	m[12] = U8TO64_LE(datablock+112);	m[28] = U8TO64_LE(datablock+240);
	m[13] = U8TO64_LE(datablock+96);	m[29] = U8TO64_LE(datablock+224);
	m[14] = U8TO64_LE(datablock+104);	m[30] = U8TO64_LE(datablock+232);
	m[15] = U8TO64_LE(datablock+120);	m[31] = U8TO64_LE(datablock+248);
	...
	TT[2] = _mm_xor_si128( TT[2], _mm_load_si128((__m128i*)(&SC512[27][0])));
	TT[3] = _mm_xor_si128( TT[3], _mm_load_si128((__m128i*)(&SC512[27][2])));
	TT[6] = _mm_xor_si128( TT[6], _mm_load_si128((__m128i*)(&SC512[27][4])));
	TT[7] = _mm_xor_si128( TT[7], _mm_load_si128((__m128i*)(&SC512[27][6])));
